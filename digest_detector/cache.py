...
def _config_snapshot() -> dict:
    ...
    return {
        ...
        "PHONETIC_MAX_SYLLABLES": config.PHONETIC_MAX_SYLLABLES,
    }
...
class PipelineCache:
    ...
    def load(self) -> tuple[list[ExtractedText], list[CandidatePair]]:
        ...
        Trust model: cache files are self-generated by save() and trusted.
        Pickle deserialization can execute arbitrary code, so these files
        should not be accepted from untrusted sources.

        Raises:
            CacheCorruptionError: If cache files are corrupted or unreadable.
        """
        try:
            ...
            raise CacheCorruptionError(
                f"Cache files corrupted or unreadable: {e}"
            ) from e
        return texts, candidates


class CacheCorruptionError(Exception):
    """Raised when cache files are corrupted or cannot be deserialized."""
